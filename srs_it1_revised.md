...
- As a beginner investor, I want to buy and sell stocks using real-time or delayed real market data with virtual money so that I can practice trading without financial risk.
...
### Nice to have:
- As a user who benefits from structure, I want to participate in weekly or monthly trading challenges with predefined constraints (ex: tech stocks only, long-only, low-volatility strategy) so that I can explore focused learning goals.
...
- Kalshi Demo - Gives demo version of its app, but lacks educational features and AI- driven feedback. Only integrates trading/betting into one platform.